 ...
 */
export function createBlockStyleEditingPlugin(presetIdentifier: CkStylesPresetIdentifier, presetConfiguration: CkStylesPresetConfiguration): PluginConstructor {
  return class BlockStyleEditing extends Plugin {
    ...
    init() {
      ...
      this.editor.conversion.attributeToAttribute({
        ...
        view: optionIdentifiers.reduce((viewConfig: Record<string, any>, optionIdentifier) => {
          ...
          if ('attribute' in options) {
            ...
            throw new Error(`Invalid configuration for option ${optionIdentifier} in preset ${presetIdentifier}: either "attribute" and "attributeValue" or "cssClass" must be set.`);
          }
      ...
      // Register command
      const commandId = `blockStyles:${presetIdentifier}`
      ...
      this.editor.ui.componentFactory.add(
        ...
        (locale) => {
          ...
          Object.keys(presetConfiguration.options).forEach(optionIdentifier => {
            ...
            itemDefinition.model.bind('isOn').to(command, 'value', (value) => value.includes(optionIdentifier));
